document.addEventListener('DOMContentLoaded', function () {
    const passwordInputs = document.querySelectorAll('#passwordInput, #passwordInputs');
    const togglePasswordIcons = document.querySelectorAll('#togglePasswordIcon, #togglePasswordIcons');

    // Iterate over both input and icon lists
    passwordInputs.forEach((passwordInput, index) => {
        const togglePasswordIcon = togglePasswordIcons[index];

        if (!togglePasswordIcon) return; // Skip if no corresponding icon
